-- â”€â”€â”€ Pricing Intelligence â€” migration 011 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- Adds: elasticity snapshots, variable importance, scientist runs, data maturity

-- Snapshot d'Ã©lasticitÃ© calculÃ© par segment (lu par le scientist + le dashboard)
create table if not exists public.price_elasticity_snapshots (
  ...
  confidence numeric(3,2) default 0,         -- 0-1, fiabilitÃ© de la courbe
...
-- Variables les plus rÃ©munÃ©ratrices ("la variable parfaite")
create table if not exists public.pricing_variable_importance (
  ...
  revenue_spread_cents integer,              -- Ã©cart de RPI entre meilleure et pire valeur
...
-- Runs du Pricing Scientist (audit + dashboard + mÃ©moire)
create table if not exists public.pricing_scientist_runs (
  ...
  actions jsonb default '[]'::jsonb,         -- candidats ajoutÃ©s/Ã©laguÃ©s
  model_used text,
  tokens_in integer,
  tokens_out integer,
  duration_ms integer,
...
-- MaturitÃ© de donnÃ©es par (plan, segment) â€” gate Claude vs modÃ¨le maison
create table if not exists public.pricing_data_maturity (
  ...
  maturity_score numeric default 0,          -- 0-1, calculÃ© via sigmoÃ¯de
  preferred_engine text default 'claude',    -- bascule Ã  'in_house_model' au-dessus du seuil
  updated_at timestamptz default now() not null,
  primary key (plan_id, segment_hash)
);

-- MÃ©moire de pricing (raisonnements durables, rÃ©injectÃ©s dans runs futurs)
